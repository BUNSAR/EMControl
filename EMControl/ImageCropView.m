...
static CGFloat const DEFAULT_MASK_ALPHA = 0.50;
static bool const square = NO;
float IMAGE_MIN_HEIGHT = 75;
float IMAGE_MIN_WIDTH = 75;
...
CGRect imageFrame;
CGFloat imageFrameScale;
NSMutableArray *bounding_box_list;
int bounding_box_index = 0;

UISwitch *genderSwitch;
UIImageView *genderSelectedImage;

- (void)setBoundingBoxes:(NSMutableArray*)arr ofIndex:(int)index {
    bounding_box_index = index;
    bounding_box_list = [[NSMutableArray alloc] init];
    [bounding_box_list removeAllObjects];
    [bounding_box_list addObjectsFromArray:arr];
}
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        CGFloat top_bar_height = [[self navigationController] navigationBar].bounds.size.height + statusBarSize.height +  20;
        CGRect view = CGRectMake(0, top_bar_height, self.view.bounds.size.width, self.view.bounds.size.height - top_bar_height - 50);
        
        UIImageView *iv = [[UIImageView alloc] initWithFrame:view];
        ...
        imageFrame = CGRectMake(roundf(0.5f*(CGRectGetWidth(iv.bounds)-scaledImageSize.width)), roundf(0.5f*(CGRectGetHeight(iv.bounds)-scaledImageSize.height)), roundf(scaledImageSize.width), roundf(scaledImageSize.height));
        imageFrameScale = imageFrame.size.width/self.image.size.width;

        NSLog(@"frm: %f,%f,%f,%f,%f", imageFrame.origin.x,f_y, f_width, f_height, imageFrameScale);
        
        BoundingBox* b = bounding_box_list[bounding_box_index];
        
        f_x      = b.x      * imageFrameScale + imageFrame.origin.x;
        f_y      = b.y      * imageFrameScale + imageFrame.origin.y;
        f_width  = b.width  * imageFrameScale;
        f_height = b.height * imageFrameScale;
        
        self.cropView  = [[ImageCropView alloc] initWithFrame:view blurOn:self.blurredBackground rectFrame:CGRectMake(f_x, f_y, f_width, f_height)];
...
- (void) initCustomProporties {
    ...
    UIImage *image = [UIImage imageNamed:@"bunsar_trans"];
    self.navigationItem.titleView = [[UIImageView alloc] initWithImage:image];
    
    CGRect viewFrame  = self.view.frame;
    int viewHeight = 50;
    
    UIButton *backButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 20, 100, 30)];
    [backButton setTitle:@"Back" forState:(UIControlStateNormal)];
    [backButton setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
    [backButton.titleLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:18.0]];
    [backButton addTarget:self action:@selector(cancel:) forControlEvents:(UIControlEventTouchUpInside)];
    [self.view addSubview:backButton];
    
    
    CGRect buttonsViewFrame = CGRectMake(0, viewFrame.size.height-viewHeight, viewFrame.size.width, viewHeight);
    UIView *buttonsView = [[UIView alloc]initWithFrame:buttonsViewFrame];
    buttonsView.backgroundColor = [UIColor whiteColor];
    
    UILabel *genderLabel = [[UILabel alloc]initWithFrame:CGRectMake(8, 0, 100, viewHeight)];
    [genderLabel setText:@"Gender"];
    [genderLabel setTextColor:[UIColor blackColor]];
    [genderLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:18.0]];
    [buttonsView addSubview:genderLabel];
    
    genderSwitch = [[UISwitch alloc] initWithFrame: CGRectMake(70, 10, 20, 20)];
    [genderSwitch addTarget: self action: @selector(switchGender:) forControlEvents:UIControlEventValueChanged];
    [buttonsView addSubview: genderSwitch];
    [genderSwitch setTintColor:[UIColor colorWithRed:0.10 green:0.56 blue:0.84 alpha:1.0]];
    [genderSwitch setOnTintColor:[UIColor colorWithRed:0.78 green:0.35 blue:0.50 alpha:1.0]];
    
    [self.view addSubview:buttonsView];
    
    genderSelectedImage = [[UIImageView alloc]initWithFrame:CGRectMake(125, 10, 40, viewHeight - 20)];
    genderSelectedImage.contentMode = UIViewContentModeScaleAspectFit;
    genderSelectedImage.clipsToBounds = YES;
    [buttonsView addSubview:genderSelectedImage];
    
    if ([gender isEqualToString:@"female"]) {
        [genderSwitch setOn:true];
        genderSelectedImage.image = [UIImage imageNamed:@"EMControl.bundle/icon_female"];
    } else {
        [genderSwitch setOn:false];
        genderSelectedImage.image = [UIImage imageNamed:@"EMControl.bundle/icon_male"];
    }
    
    UIButton *searchButton = [[UIButton alloc]initWithFrame:CGRectMake(buttonsViewFrame.size.width - 110, 0, 110, viewHeight)];
    [searchButton setTitle:@"Search" forState:(UIControlStateNormal)];
    [searchButton setTitleColor:[UIColor blackColor] forState:(UIControlStateNormal)];
    searchButton.tag = 2;
    [searchButton.titleLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:18.0]];
    [searchButton addTarget:self action:@selector(doneWithGender:) forControlEvents:(UIControlEventTouchUpInside)];
    [buttonsView addSubview:searchButton];
    
    UIImageView *searcImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 35, 35)];
    searcImage.center = CGPointMake(8, searchButton.frame.size.height / 2);
    searcImage.image = [UIImage imageNamed:@"EMControl.bundle/icon_search"];
    [searchButton addSubview:searcImage];
    ...
    int index = 0;
    for (BoundingBox *b in bounding_box_list) {
        UIButton *boundingAreaButton = [[UIButton alloc]initWithFrame:CGRectMake(
                b.x * imageFrameScale + imageFrame.origin.x + b.width*imageFrameScale/2 - 20,
                b.y * imageFrameScale + imageFrame.origin.y + top_bar_height + b.height*imageFrameScale/2 - 20,
                40, 40)];
        
        [boundingAreaButton setTitle:[NSString stringWithFormat:@"%d", index+1] forState:(UIControlStateNormal)];
        [boundingAreaButton setTitleColor:[UIColor darkGrayColor] forState:(UIControlStateNormal)];
        [boundingAreaButton setBackgroundColor:[UIColor clearColor]];
        [boundingAreaButton setBackgroundImage:[UIImage imageNamed:@"EMControl.bundle/icon_bounding_button"] forState:UIControlStateNormal];
        boundingAreaButton.tag = index;
        [boundingAreaButton addTarget:self action:@selector(reSetBoundingBox:) forControlEvents:(UIControlEventTouchUpInside)];
        [boundingAreaButton.titleLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:18.0]];
        [self.view addSubview:boundingAreaButton];
        index = index+1;
        
        NSLog(@"button %d: %f,%f,%f,%f",index-1,b.x,b.y,b.width, b.height);
    }
}

- (void)reSetBoundingBox:(UIButton *)sender{
    bounding_box_index  = (int) sender.tag;
    bounding_box_index = bounding_box_index == bounding_box_list.count ? 0 : bounding_box_index;
...
- (IBAction)switchGender:(id)sender {
    if (genderSwitch.on) {
        genderSelectedImage.image = [UIImage imageNamed:@"female"];
        gender = @"female";
        NSLog(@"female");
    } else{
        genderSelectedImage.image = [UIImage imageNamed:@"male"];
        gender = @"male";
        NSLog(@"male");
    }
}

- (void)doneWithGender:(UIButton *)sender{
    if ([self.delegate respondsToSelector:@selector(ImageCropViewControllerSuccess:didFinishCroppingImage:cropRect:gender:)])
    {
        UIImage *cropped;
        CGRect CropRect;
        if (self.image != nil){
            CropRect = self.cropView.cropAreaInImage;
        ...
        [self.delegate ImageCropViewControllerSuccess:self didFinishCroppingImage:cropped cropRect:CropRect gender:gender];
    }
}
...
static CGFloat const DEFAULT_CONTROL_POINT_SIZE = 15;
...
- (id)initWithFrame:(CGRect)frame blurOn:(BOOL)blurOn rectFrame:(CGRect)rectFrame
{
    self = [super initWithFrame:frame];
    if (self) {
        self.blurred = blurOn;
        self.sizes = rectFrame;
        [self initViews];
    }
    return self;
}
...
- (void)initViews {
    ...
    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(self.sizes.origin.x ,
                                                                   self.sizes.origin.y ,
                                                                   controlPointSize)];

    bottomLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(self.sizes.origin.x,
                                                                      self.sizes.origin.y+self.sizes.size.height,
                                                                      controlPointSize)];
    
    bottomRightPoint = [self createControlPointAt:SquareCGRectAtCenter(self.sizes.origin.x+self.sizes.size.width,
                                                                       self.sizes.origin.y+self.sizes.size.height,
                                                                       controlPointSize) ];
    
    topRightPoint = [self createControlPointAt:SquareCGRectAtCenter(self.sizes.origin.x+self.sizes.size.width,
                                                                    self.sizes.origin.y,
                                                                    controlPointSize)];
...
- (void)setImage:(UIImage *)image {
    CGFloat f_widthidth = self.frame.size.width;
    CGFloat f_heighteight = self.frame.size.height;
    CGFloat imageWidth = image.size.width;
    CGFloat imageHeight = image.size.height;
    BOOL isPortrait = imageHeight / f_heighteight > imageWidth/f_widthidth;
    int x, y;
    int scaledImageWidth, scaledImageHeight;
    if (isPortrait) {
        imageScale = imageHeight / f_heighteight;
        scaledImageWidth = imageWidth / imageScale;
        scaledImageHeight = f_heighteight;
        x = (f_widthidth - scaledImageWidth) / 2;
        y = 0;
    }
    else {
        imageScale = imageWidth / f_widthidth;
        scaledImageWidth = f_widthidth;
        scaledImageHeight = imageHeight / imageScale;
        x = 0;
        y = (f_heighteight - scaledImageHeight) / 2;
    }
    ...
    /*
    if (self.blurred) {
        blur = [image blurredImageWithRadius:15 iterations:1 tintColor:[UIColor blackColor]];
    } else {
